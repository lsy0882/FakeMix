...
import json
import pandas as pd
from fake_celeb_dataset import FakeAVceleb, FakeMix
...
def test_for_fakemix(dist_model, avfeature_model, loader, dist_reg_model, avfeature_reg_model, max_len=50):
    output_dir = opts.output_dir
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    logger_path = os.path.join(output_dir, 'output.log')
    score_file_path = os.path.join(output_dir, 'testing_scores.npy')
    score_history_path = os.path.join(output_dir, 'tools_for_FakeMix/testing_scores.json')
    logger = get_logger(logger_path)
    logger.info('Start testing!')
    score_list = []
    category_list = []
    output_record = {}
    total_n_clips = 102026
    clip_count = 0
    with logging_redirect_tqdm():
        # with tqdm(total=len(loader), position=0, leave=False, colour='green', ncols=150) as pbar:
        with tqdm(total=total_n_clips, position=0, leave=False, colour='green', ncols=150) as pbar:
            for nm, aud_vises in enumerate(loader):
                for aud_vis in aud_vises:
                    clip_count += 1
                    video_set = aud_vis['video']
                    audio_set = aud_vis['audio']
                    path_for_detect = aud_vis['sample']
                    video_name = str(aud_vis['sample'][0]) + "_sec_" + str(aud_vis['second'].item())
                    annotation = aud_vis['category']
                    #print(path_for_detect)
                    pbar.set_postfix(data_path = path_for_detect)
                    time_len = video_set.shape[2]
                    #predict_set = np.zeros((time_len - 5 + 1, 31))
                    ...
                    #tqdm.write("The score of this video is {} ".format(prob.item()))
                    logger.info("The score of this video is {} ".format(prob.item()))
                    output_record[video_name] = (annotation[0], prob.item())
                    if clip_count % 100 == 0:
                        save_to_json(score_history_path, output_record)
                    # score_list.append(prob.item())
                    # category_list.append(annotation)
                    pbar.update(1) # each clip finish!
            # np.save(score_file_path, np.array(score_list))
            # with open(score_history_path, 'w') as file:
            #     json.dump(output_record, file, indent=4)
            if clip_count % 100 != 0:
                save_to_json(score_history_path, output_record)
            logger.info('Finished!')
...
def main():
    ...
    if opts.test_video_path is not None:
        if opts.test_video_path.split('.')[-1] == 'mp4':
            test_video = FakeMix([opts.test_video_path], opts.resize, opts.fps, opts.sample_rate, vid_len=opts.vid_len, phase=0, train=False, number_sample=1, lrs2=False, need_shift=False, lrs3=False, kodf=False, lavdf=False, robustness=False, test=True)
        else:
            with open(opts.test_video_path) as file:
                test_video_path = file.readlines()
            test_video = FakeMix(test_video_path, opts.resize, opts.fps, opts.sample_rate, vid_len=opts.vid_len, phase=0, train=False, number_sample=1, lrs2=False, need_shift=False, lrs3=False, kodf=False, lavdf=False, robustness=False, test=True)
    loader_test = DataLoader(test_video, batch_size=opts.bs, num_workers=opts.n_workers, shuffle=False)
    test_for_fakemix(sync_model, avfeature_sync_model, loader_test, dist_reg_model=dist_regressive_model, avfeature_reg_model=avfeature_regressive_model, max_len=opts.max_len)
