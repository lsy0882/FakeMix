...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, data_partition, preprocessing_func):
        ...
        if self.data_partition == "train":
            ...
            for category in tqdm(os.listdir(self.data_path), desc='Loading categories'):
                ...
                type_video_path = os.path.join(category_path, 'clips')

                for person_id in tqdm(os.listdir(type_video_path), desc=f'Processing clips', leave=False):
                    person_clips_path = os.path.join(type_video_path, person_id)

                    for file_id in os.listdir(person_clips_path):
                        file_video_path = os.path.join(person_clips_path, file_id)
                        video_files = sorted([f for f in os.listdir(file_video_path) if f.endswith('.mp4')])

                        for i in range(len(video_files)):
                            clip_video_path = os.path.join(file_video_path, video_files[i])
        ...
        # Load test dataset
            ...
            for category in tqdm(os.listdir(self.data_path), desc='Loading categories'):
                ...
                for person_id in tqdm(os.listdir(category_path), desc=f'Processing clips', leave=False):
                    ...
                    full_video_paths = sorted([f for f in os.listdir(person_id_path) if f.endswith('.mp4')])
                    annotation_paths = sorted([f for f in os.listdir(person_id_path) if f.endswith('.json')])
                    for idx in range(len(full_video_paths)):
                        full_video_path = os.path.join(person_id_path, full_video_paths[idx])
    ...
    def __getitem__(self, idx):
        ...
        if self.data_partition == "train":
            clip_video_path, video_class = self.samples[idx]
            video_frames, fps = self.process_video(clip_video_path)
            return {
