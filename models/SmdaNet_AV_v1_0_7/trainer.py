...
class Trainer(object):
    ...
    def _train(self, dataloader):
        ...
        for idx, batch in enumerate(dataloader):
            ...
            frames_mfcc_label = batch["video_mfcc_class"] # torch.Size([B,])
            ...
            frames_mfcc_label = frames_mfcc_label.to(self.device)

            with torch.cuda.amp.autocast():
                outputs = self.model(frames, mfccs) # outputs = tuple(weigted_v_softmax, weigted_a_softmax)
                loss = self.criterion(outputs, frames_mfcc_label)
            ...
            num_correct += int((torch.argmax(outputs, axis=1) == frames_mfcc_label).sum())


            pbar.set_postfix(loss="{:.08f}".format(float(train_loss / (B * (idx + 1)))),
                            av_acc="{:.04f}%".format(num_correct / (B * (idx + 1)) * 100),
                            lr="{:.04f}".format(float(self.optimizer.param_groups[0]['lr'])))
            pbar.update()
        pbar.close()
        train_loss = train_loss / (B * len(dataloader))
        train_av_acc = num_correct / (B * len(dataloader)) * 100
        
        return train_loss, train_av_acc
    ...
    def _validate(self, dataloader):
        ...
        with torch.inference_mode():
            ...
            val_av_acc = num_correct / (B * len(dataloader)) * 100
            
            return val_loss, val_av_acc
            
    def _test(self, dataloader):
        self.model.eval()
        test_loss = 0
        total_av_correct, total_av_num, avg_each_av_acc = 0, 0, 0
        records = []
        ...
        with torch.inference_mode():
            for idx, batch in enumerate(dataloader):
                ...
                videos_mfcc_label = batch["video_mfcc_class"] # torch.Size([B, T])
                
                ### Move Data to Device (Ideally GPU)
                videos = videos.to(self.device)
                mfccs = mfccs.to(self.device)
                videos_mfcc_label = videos_mfcc_label.to(self.device)
                record_label = videos_mfcc_label[0].tolist()
                preds = []
                ...
                for i in range(T):
                    ...
                    current_video_mfcc_label = videos_mfcc_label[0][i]

                    outputs = self.model(video_pair, mfcc_pair) # outputs = tuple(weigted_v_softmax, weigted_a_softmax)
                    loss = self.criterion(outputs, current_video_mfcc_label.unsqueeze(dim=0))
                        
                    current_correct += int((torch.argmax(outputs, axis=1) == current_video_mfcc_label).sum())
                    test_loss += loss.item()
                    
                    preds.append(torch.argmax(outputs, axis=1))
                
                current_acc = (current_correct / (B * T)) * 100
                avg_each_av_acc += current_acc
                total_av_correct += current_correct
                total_av_num += (B * T)
                
                # Append results to the records list
                for video_path, video_acc in zip([video_path], [current_acc]):
                    v_name = os.path.basename(video_path)
                    v_label = record_label[1:]
                    v_pred = [pred.item() for pred in preds]
            ...
            avg_each_av_acc /= len(dataloader)
            total_av_acc = total_av_correct / total_av_num * 100

        return test_loss, avg_each_av_acc, total_av_acc
    ...
    def run(self):
        ...
        with torch.cuda.device(self.gpuid):
            ...
            for epoch in range(self.start_epoch, self.config['engine']['epoch']):
                ...
                train_loss, train_av_acc = self._train(self.dataloaders['train'])
                # val_loss, val_av_acc = self._validate(self.dataloaders['val'])
                ...
                logger.info(f"\t Train Loss: {train_loss:04f} \t Train Audio-Video Accuracy: {train_av_acc:04f} \t Learning Rate {curr_lr:07f}")
                # logger.info(f"\t Valid Loss: {val_loss:04f} \t Valid Audio-Video Accuracy: {val_av_acc:04f} \t Learning Rate {curr_lr:07f}")

                # Log metrics at each epoch using wandb
                self.wandb_run.log({'Learning Rate': curr_lr})
                self.wandb_run.log({'Train Loss': train_loss, "Train Audio-Video Accuracy": train_av_acc})
                # self.wandb_run.log({'Valid Loss': val_loss, "Valid Audio-Video Accuracy": val_av_acc})
                ...
                if epoch % 5 == 0:
                    test_loss, avg_each_av_acc, total_av_acc = self._test(self.dataloaders['test'])
                    
                    logger.info("\t Test loss: {:.08f}".format(test_loss))
                    logger.info("\t Average each accuracy: {:.04f}".format(avg_each_av_acc))
                    logger.info("\t Total accuracy: {:.04f}".format(total_av_acc))

                    # Log metrics at each epoch using wandb
                    self.wandb_run.log({'Test loss': test_loss})
                    self.wandb_run.log({'Average each accuracy': avg_each_av_acc})
                    self.wandb_run.log({'Total accuracy': total_av_acc})