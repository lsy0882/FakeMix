...
class Trainer(object):
    ...
    def _train(self, dataloader):
        self.model.train()
        train_loss, video_num_correct = 0, 0
        ...
        for idx, batch in enumerate(dataloader):
            ...
            with torch.cuda.amp.autocast():
                ...
                v_loss = self.criterion(outputs, frames_label)
                
            self.scaler.scale(v_loss).backward() # This is a replacement for loss.backward()
            self.scaler.step(self.optimizer) # This is a replacement for optimizer.step()
            self.scaler.update()

            ### Calculate performance matrics
            train_loss += v_loss.item()
            video_num_correct += int((torch.argmax(outputs, axis=1) == frames_label).sum())
        ...
        return train_loss, train_v_acc
        
    def _validate(self, dataloader):
        self.model.eval()
        val_loss, video_num_correct = 0, 0
    ...
    def run(self):
        ...
        with torch.cuda.device(self.gpuid):
            ...
            for epoch in range(self.start_epoch, self.config['engine']['epoch']):
                ...
                train_loss, train_v_acc = self._train(self.dataloaders['train'])
                # val_loss, val_v_acc = self._validate(self.dataloaders['val'])
                ...
                logger.info(f"\t Train Loss: {train_loss:04f} \t Train Video Accuracy: {train_v_acc:04f} \t Learning Rate {curr_lr:07f}")
                # logger.info(f"\t Valid Loss: {val_loss:04f} \t Valid Video Accuracy: {val_v_acc:04f} \t Learning Rate {curr_lr:07f}")

                # Log metrics at each epoch using wandb
                self.wandb_run.log({'Learning Rate': curr_lr})
                self.wandb_run.log({'Train Loss': train_loss, "Train Video Accuracy": train_v_acc})
                # self.wandb_run.log({'Valid Loss': val_loss, "Valid Video Accuracy": val_v_acc})
